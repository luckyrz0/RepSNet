...
#            self.dropout.p = self.p / self.steps * self.current_step
#            self.current_step += 1
#         x = self.dropout(x)
#         return x
...
class RepSNet(nn.Module):
    ...
    def __init__(
        ...
        if not self.pretrained:
            self.nucleus_map = nn.Sequential(nn.Conv2d(filters[0], 2, 1, stride=1, padding=0, bias=True), )
            self.type_map = nn.Sequential(nn.Conv2d(filters[0], self.num_classes, 1, stride=1, padding=0, bias=True), )
        self.boundary_distance_map = nn.Sequential(nn.Conv2d(filters[0], 4, 1, stride=1, padding=0, bias=True),
                                                   nn.ReLU())
    ...
    def get_loss(self, out, feed_dict):
        ...
        if not self.pretrained:
            ...
            # weight = torch.bincount(type_true.view(-1), minlength=7)
            # weight = (1 / weight).nan_to_num(0)
            # weight = weight / weight.sum() + 1 / 7
        ...
        type_true_bool = type_true > 0
        boundary_distance_pred_2 = boundary_distance_pred ** 2
        # loss_dict["consistency_loss"] = (F.mse_loss(boundary_distance_pred[:,0,:,:-1]**2,boundary_distance_pred[:,0,:,1:]**2+1,reduction='none')[type_true_bool[:,:,:-1] & type_true_bool[:,:,1:]]).mean() + \
        #                                 (F.mse_loss(boundary_distance_pred[:,1,:,:-1]**2+1,boundary_distance_pred[:,1,:,1:]**2,reduction='none')[type_true_bool[:,:,:-1] & type_true_bool[:,:,1:]]).mean() + \
        #                                 (F.mse_loss(boundary_distance_pred[:,2,:-1,:]**2,boundary_distance_pred[:,0,2:,:]**2+1,reduction='none')[type_true_bool[:,:-1,:] & type_true_bool[:,1:,:]]).mean() + \
        #                                 (F.mse_loss(boundary_distance_pred[:,3,:-1,:]**2+1,boundary_distance_pred[:,3,1:,:]**2,reduction='none')[type_true_bool[:,:-1,:] & type_true_bool[:,1:,:]]).mean()
        dl = (boundary_distance_pred_2[:, 0, :, :-1] - boundary_distance_pred_2[:, 0, :, 1:] + 1)[
            type_true_bool[:, :, :-1] & type_true_bool[:, :, 1:]]
        dr = (boundary_distance_pred_2[:, 1, :, :-1] - boundary_distance_pred_2[:, 1, :, 1:] - 1)[
            type_true_bool[:, :, :-1] & type_true_bool[:, :, 1:]]
        du = (boundary_distance_pred_2[:, 2, :-1, :] - boundary_distance_pred_2[:, 2, 1:, :] + 1)[
            type_true_bool[:, :-1, :] & type_true_bool[:, 1:, :]]
        dd = (boundary_distance_pred_2[:, 3, :-1, :] - boundary_distance_pred_2[:, 3, 1:, :] - 1)[
            type_true_bool[:, :-1, :] & type_true_bool[:, 1:, :]]
        dl, dr, du, dd = dl[(1 > dl) & (dl > -1)], dr[(1 > dr) & (dr > -1)], du[(1 > du) & (du > -1)], dd[
            (1 > dd) & (dd > -1)]
        # print(dl.mean().item(), dr.mean().item(), du.mean().item(), dd.mean().item())

        loss_dict["consistency_loss"] = (dl ** 2).mean() + (dr ** 2).mean() + (du ** 2).mean() + (dd ** 2).mean()
        # loss_dict["consistency_loss"] = (dl**2).mean() + (dr**2).mean()
        ############
    ...
    def get_ann(self, out, net_args={}):
        ...
        ################################边界聚合操作
        # new_boundary_map = boundary_map.unsqueeze(1).clone()

        # new_boundary_map = new_boundary_map.float()
        # keneral = torch.ones((1,1,3,3))

        # device = "cuda" if torch.cuda.is_available() else "cpu"

        # keneral = keneral.to(device=device)

        # with torch.no_grad():
        #     new_boundary_map = F.conv2d(
        #               input=new_boundary_map,weight=keneral,stride=1,padding=1
        #         )

        # new_boundary_map = new_boundary_map.squeeze()

        ################################

        # boundary_map = boundary_map.to("cpu").data.numpy()

        # boundary_map = boundary_map >= boundary_threshold-1

        ###################
        # boundary_map = boundary_map > 0
        # boundary_distance_map = out["boundary_distance_map"].cpu().numpy()**2
        # boundary_distance_map = (boundary_distance_map + 0.5).astype(np.int32)
        ###################
        ...
        for b in range(B):
            ...
            if X_instance_index.shape[0] > 0:
                ...
                ################### 2023 07 20 二段自适应阈值 add
                # boundary_map_B = instance_map - instance_map_save
                # boundary_map_B[X_boundary_index[vlaid_boundary_index, 0], X_boundary_index[vlaid_boundary_index, 1]] = Y_instance[neigh_ind[vlaid_boundary_index]].reshape(-1)
                # boundary_map_vote_B = np.asarray(boundary_map_vote[b], dtype=np.float32)
                # for i in np.unique(boundary_map_B)[1:]:
                #     instance_boundary = boundary_map_B == i
                #     # instance_inner = instance_map_save == i
                #     # h, v = np.zeros_like(boundary_map_B), np.zeros_like(boundary_map_B)
                #     # h[instance_inner] = boundary_distance_map[b, 1, instance_inner] + boundary_distance_map[b, 0, instance_inner] + 1
                #     # v[instance_inner] = boundary_distance_map[b, 3, instance_inner] + boundary_distance_map[b, 2, instance_inner] + 1
                #     # h = np.sqrt(np.sum(h, axis=1))
                #     # v = np.sqrt(np.sum(v, axis=0))
                #     # instance_boundary_index = np.vstack(np.where(instance_boundary))
                #     # boundary_map_vote_B[instance_boundary_index[0], instance_boundary_index[1]] /= h[instance_boundary_index[0]] + v[instance_boundary_index[1]]
                #     instance_mask = instance_map == i
                #     radius = np.sqrt(instance_mask.sum() / np.pi)
                #     instance_boundary_index = np.vstack(np.where(instance_boundary))
                #     boundary_map_vote_B[instance_boundary_index[0], instance_boundary_index[1]] /= radius
                # boundary_map_vote_B = boundary_map_vote_B > 0.6  # 暂定
                # # boundary_map_vote_B[boundary_map_B == 0] = 0 # 更差了
                # instance_map, _ = ndimage.label(1 - boundary_map_vote_B)
                # instance_map -= 1
                # instance_map[instance_map == -1] = 0
                # instance_map_unique, instance_map_count, = np.unique(instance_map, return_counts=True)
                # for invalid_instance in instance_map_unique[instance_map_count <= instance_threshold[0]]:
                ...
                # if X_instance_index.shape[0] > 0:
                #     Neigh.fit(X_instance_index)
                #     X_boundary_index = np.vstack(np.where(boundary_map_vote_B)).T
                #     neigh_dist, neigh_ind = Neigh.kneighbors(X_boundary_index, return_distance=True)
                #     vlaid_boundary_index = np.where(neigh_dist <= neigh_dist_threshold)[0]
