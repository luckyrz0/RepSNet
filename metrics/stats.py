...
def get_aji(true, pred, remap=True):
    ...
    if pairwise_iou.shape[0] == 0 and pairwise_iou.shape[1] == 0:
        return 1
    elif pairwise_iou.shape[0] == 0 or pairwise_iou.shape[1] == 0:
        return 0
...
def compute_stats(pred_array, true_array, num_classes=7):

    type_flag = true_array.shape[-1] > 1

    aji_list = []
    dice_list = []
    pq_list = []
    mpq_info_list = []
    for idx in tqdm(range(pred_array.shape[0])):
        ...
        aji_list.append(get_aji(true_inst, pred_inst))
        dice_list.append(get_dice(true_inst, pred_inst))
        pq_list.append(get_pq(true_inst, pred_inst)[0][2])

        if type_flag:
            mpq_info = []
            # aggregate the stat info per class
            for single_class_pq in get_multi_pq_info(true, pred, num_classes-1):
    ...
    all_metrics = {}
    all_metrics["aji"] = np.mean(aji_list)
    all_metrics["dice"] = np.mean(dice_list)
    all_metrics["pq"] = np.mean(pq_list)
    if type_flag:
        total_mpq_info_metrics = np.sum(np.array(mpq_info_list, dtype="float"), axis=0)
        mpq_list = []
        ...
        all_metrics["mpq"] = np.mean(mpq_list)
        
    for key in all_metrics.keys():
        print("%s=%.4f|"%(key, all_metrics[key]), end='')
    print()
...
if __name__ == "__main__":
    ...
    other_info = pickle.load(open("./other_info.pkl", 'rb'))
    
    stats = compute_stats(other_info["pred_array"][:100], other_info["true_array"][:100], num_classes=7)
